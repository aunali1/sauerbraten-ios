	...
	const bool bRotate)
{
	...
	m_fScreenScale[0] = (bRotate ? fY : fX) /kDeviceX;
	m_fScreenScale[1] = (bRotate ? fX : fY) /kDeviceY;
	...
	*/
	...
	bStatus = APIUpLoadIcons(PVRTPrint3DPVRLogo, PVRTPrint3DIMGLogo);
	if (!bStatus) return false;
...
*****************************************************************************/
bool CPVRTPrint3D::Print3D(float fPosX, float fPosY, const float fScale, unsigned int Colour, const char * const pszFormat, ...)
{
#if !defined (DISABLE_PRINT3D)
...
/*!***************************************************************************
 ...
 @Description		Creates a default title with predefined position and colours.
					...
					2 = Img Tech logo
*****************************************************************************/
bool CPVRTPrint3D::DisplayDefaultTitle(const char * const pszTitle, const char * const pszDescription, const unsigned int uDisplayLogo)
{
	bool bRet = true;
	...
	if(pszTitle)
	{
		bRet &= Print3D(0.0f, 1.0f, 1.2f,  PVRTRGBA(255, 255, 0, 255), pszTitle);
	}
	...
	if(pszDescription)
	{
		bRet &= Print3D(0.0f, 8.0f, 0.9f,  PVRTRGBA(255, 255, 255, 255), pszDescription);
	}
...
unsigned int CPVRTPrint3D::CreateDefaultWindow(float fPosX, float fPosY, int nXSize_LettersPerLine, char *sTitle, char *sBody)
{
	...
	/* Set title */
	if (sTitle)
		SetTitle(dwActualWin, PVRTRGBA(0x20, 0x20, 0xB0, 0xE0), 0.6f, PVRTRGBA(0xFF, 0xFF, 0x30, 0xFF), sTitle, PVRTRGBA(0xFF, 0xFF, 0x30, 0xFF), "");
...
*****************************************************************************/
bool CPVRTPrint3D::DisplayWindow(unsigned int dwWin)
{
#if !defined (DISABLE_PRINT3D)
...
/*!***************************************************************************
 ...
 @Description		Feed the text buffer of window referenced by dwWin.
					This function accepts formatting in the printf way.
*****************************************************************************/
bool CPVRTPrint3D::SetText(unsigned int dwWin, const char *Format, ...)
{
#if !defined (DISABLE_PRINT3D)
						  ...
						  float fPosX, float fPosY, float fSizeX, float fSizeY)
{
	...
	if(	m_pWin[dwWin].fWinFontSize		!= fFontSize ||
		...
		m_pWin[dwWin].fWinPos[0]		!= fPosX  * kDeviceY/100.0f ||
		m_pWin[dwWin].fWinPos[1]		!= fPosY  * kDeviceX/100.0f ||
		m_pWin[dwWin].fWinSize[0]		!= fSizeX * kDeviceY/100.0f ||
		m_pWin[dwWin].fWinSize[1]		!= fSizeY * kDeviceX/100.0f)
	{
		/* Set window properties */
	...
	const char	* sString)
{
	...
	else /* System font */
	{
		...
		while (Val)
		{
			...
			fSize += PVRTPrint3DSize_System[Val]  * fScale * (100.0f / 480.0f);
			Val = *sString++;
		}
...
void CPVRTPrint3D::GetAspectRatio(unsigned int *dwScreenX, unsigned int *dwScreenY)
{
	...
	*dwScreenX = (int)(kDeviceY * m_fScreenScale[0]);
	*dwScreenY = (int)(kDeviceX * m_fScreenScale[1]);
...
void CPVRTPrint3D::Rotate(SPVRTPrint3DAPIVertex * const pv, const unsigned int nCnt)
{
	...
	for(i = 0; i < nCnt; ++i)
	{
		x = VERTTYPEDIV((VERTTYPE&)pv[i].sx, f2vt(kDeviceY * m_fScreenScale[0]));
		y = VERTTYPEDIV((VERTTYPE&)pv[i].sy, f2vt(kDeviceX * m_fScreenScale[1]));
		(VERTTYPE&)pv[i].sx = VERTTYPEMUL(y, f2vt(kDeviceY * m_fScreenScale[0]));
		(VERTTYPE&)pv[i].sy = VERTTYPEMUL(f2vt(1.0f) - x, f2vt(kDeviceX * m_fScreenScale[1]));
	}
}
