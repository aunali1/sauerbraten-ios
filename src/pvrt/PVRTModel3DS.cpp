...
*****************************************************************************/
bool CPVRT3DSScene::ReadFromFile(const char * const pszFileName)
{
#define BREAD(Destination, Pointer, Size)	{ memcpy(Destination, Pointer, Size); Pointer+=Size; }
	...
	***************************************************/
...
#endif
	{
		...
		if (!p3DSFile)
		{
			_RPT0(_CRT_WARN, "PVRTModel3DSRead : Not enough memory to store 3DS model\n");
			fclose(Fich);
			return false;
		}
	...
	***********************************************************/
	...
	if(Chunk != M3DMAGIC)
	{
		_RPT0(_CRT_WARN, "PVRTModel3DSRead : This file is not a valid 3D Studio File.\n");
    ...
    while(FilePos < FileSize && ++SecCont < 200)
	{
		...
		switch(Chunk)
		{
			...
			default:
				if (DIAGNOSTICS)
				{
					_RPT0(_CRT_WARN,  "PVRTModel3DSRead : Unknown M3DMAGIC chunk encountered.\n");
				}
		}
	...
	{
		_RPT0(_CRT_WARN,  "PVRTModel3DSRead : ERROR! MDATA chunk not found.\n");
	...
	{
		_RPT0(_CRT_WARN,  "PVRTModel3DSRead : Model does not contain animation.\n");
	}
	...
	for (i = 0; i < nNumMesh; ++i)
	{
		...
		while(true)
		{
			...
			case POINT_ARRAY:	/* Read Number of vertices in mesh */
								...
								{
									_RPT0(_CRT_WARN,  "PVRTModel3DSRead : Not enough memory for object geometry\n");
									Destroy();
...
#ifdef	PVRT_FIXED_POINT_ENABLE	//If fixed point is enabled then convert the float values to fixed
	...
	***********/
	if (nNumLight)
	{
		...
		if (!pLight)
		{
			_RPT0(_CRT_WARN,  "PVRTModel3DSRead : Not enough memory to allocate light structures\n");
			Destroy();
	...
	************/
	if (nNumCamera)
	{
		...
		if (!pCamera)
		{
			_RPT0(_CRT_WARN,  "PVRTModel3DSRead : Not enough memory to allocate camera structures\n");
			Destroy();
	...
	********************************/
	...
	if (KFDATAFileOffset!=-1)
	{
		...
		for (i=0; i<nNumMesh; i++)
		{
			...
			************************************************/
			memset(&InversionMatrix, 0, sizeof(PVRTMATRIX));
			...
			*****************************************/
			/* Get source pointer */
			pSource = (PVRTVECTOR3 *)pMesh[i].pVertex;
...
void CPVRT3DSScene::SetFrame(const VERTTYPE fFrameNo)
{
	fFrame = fFrameNo;
...
void CPVRT3DSScene::Destroy()
{
	...
	if(pNode)
	{
		...
		for(i = 0; i < nNumNodes; ++i)
		{
			...
			FREE(pNode[i].pScaling);
			FREE(pNode[i].pRotation);
			FREE(pNode[i].pPosition);
		}
	}
	...
	if(pMesh)
	{
		...
		{
			/* Free all mesh data */
			FREE(pMesh[i].pVertex);
			FREE(pMesh[i].pFaces);
			FREE(pMesh[i].pUV);
			FREE(pMesh[i].pNormals);
...
/*!***************************************************************************
 ...
 @Description		Return position and target required to animate this camera at the
					...
					this frame.
*****************************************************************************/
bool CPVRT3DSScene::GetCamera(PVRTVECTOR3	&vFrom, PVRTVECTOR3 &vTo, const unsigned int nIdx) const
{
	int			j;
	int			nKey1, nKey2;
	VERTTYPE	t;
		...
		else
		{
			...
			for(j = 0; j < pCameraNode->PositionKeys - 1; ++j)
			{
				...
				if(nKey1 <= (int) vt2f(fFrame) && nKey2 >= (int) vt2f(fFrame))
				{
					...
					PVRTMatrixVec3Lerp(vFrom, pCameraNode->pPosition[j].p, pCameraNode->pPosition[j+1].p, t);
				} // j loop
			}
		}
	}
		...
		else
		{
			...
			for(j = 0; j < pTargetNode->PositionKeys - 1; ++j)
			{
				...
				if(nKey1 <= (int) vt2f(fFrame) && nKey2 >= (int) vt2f(fFrame))
				{
					...
					/* Get interpolated position */
					PVRTMatrixVec3Lerp(vTo, pTargetNode->pPosition[j].p, pTargetNode->pPosition[j+1].p, t);
				} // j loop
			}
		}
	}
...
/*!***************************************************************************
 ...
 @Description		Return animation matrix required to animate this mesh at the
					...
					this frame.
*****************************************************************************/
void CPVRT3DSScene::GetTransformationMatrix(PVRTMATRIX	&mOut, const S3DSMesh &sMesh)
{
...
*****************************************************************************/
void CPVRT3DSScene::DisplayInfo()
{
	int		i;
	char	pszTmp[1024];
...
*****************************************************************************/
void CPVRT3DSScene::Scale(const VERTTYPE fScale)
{
	int i, j;
...
*****************************************************************************/
PVRTMATRIX CPVRT3DSScene::GetHierarchyMatrix(short nNode, VERTTYPE fFrameNumber, PVRTMATRIX CurrentMatrix)
{
	...
	*********************/
	...
	else
	{
		...
		else
		{
			...
			/* Get interpolated position */
			PVRTMatrixVec3Lerp(Position, pNode[nNode].pPosition[nPreviousKey].p, pNode[nNode].pPosition[nNextKey].p, t);
		}
	}
	...
	********************/
	...
	else
	{
		...
		else
		{
			...
			PVRTMatrixVec3Lerp(Scaling, pNode[nNode].pScaling[nPreviousKey].s, pNode[nNode].pScaling[nNextKey].s, t);
		}
	}
...
*****************************************************************************/
void CPVRT3DSScene::GetAbsoluteRotation(PVRTMATRIX * const pmRot, VERTTYPE fFrameNumber, S3DSNode *pNode)
{
...
/*!***************************************************************************
 ...
 @Description		Compute the normal to the triangle defined by the vertices V1,
					V2 and V3.
*****************************************************************************/
PVRTVECTOR3f CPVRT3DSScene::Normal(VERTTYPE *pV1, VERTTYPE *pV2, VERTTYPE *pV3)
{
	/*
		The calculation of the normal will be done in floating point,
		doesn't matter if we're using fixed point.
	*/
	PVRTVECTOR3f fNormal;
	PVRTVECTOR3f fV1, fV2, fV3;
	PVRTVECTOR3f Vect1, Vect2;
...
*****************************************************************************/
void CPVRT3DSScene::CalculateNormals(int nNumVertex, VERTTYPE *pVertex,
