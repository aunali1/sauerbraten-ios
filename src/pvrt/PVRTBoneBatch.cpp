			...
			for(j = 0; j < 3; ++j)
			{
				...
				*/
				for(k = 0; k < (int)pvDup[wSrcIdx].size(); ++k)
				{
					...
					PVRTDataTypeRead(&vWeight2, &pV2[nOffsetWeight], eTypeWeight, nVertexBones);
					PVRTDataTypeRead(&vIdx2, &pV2[nOffsetIdx], eTypeIdx, nVertexBones);
				...
				*/
				...
				PVRTDataTypeWrite(&pVtxBuf->last()[nOffsetIdx], eTypeIdx, nVertexBones, &vIdx);
