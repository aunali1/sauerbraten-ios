...
static void scaletexture(uchar *src, uint sw, uint sh, uint bpp, uint pitch, uchar *dst, uint dw, uint dh) {
	...
	if(sw < dw || sh < dh || sw&(sw-1) || sh&(sh-1)) {
        switch(bpp) {
...
void resizetexture(int w, int h, bool mipmap, bool canreduce, GLenum target, int compress, int &tw, int &th)
{
    ...
    w = min(w, sizelimit);
    h = min(h, sizelimit);
    if((!hasNP2 || !usenp2) && target!=GL_TEXTURE_RECTANGLE_ARB_ && (w&(w-1) || h&(h-1)))
    {
        tw = th = 1;
        while(tw < w) tw *= 2;
        while(th < h) th *= 2;
...
#ifdef WIN32
			if(target==GL_TEXTURE_1D) {
				glCompressedTexImage1D_(target, level, format, w, 0, size, data);
			} else {
				glCompressedTexImage2D_(target, level, format, w, h, 0, size, data);
			}
#else
			glCompressedTexImage2D_(target, level, format, w, h, 0, size, data);
#endif
...
void createtexture(int tnum, int w, int h, void *pixels, int clamp, int filter, GLenum component, GLenum subtarget, int pw, int ph, int pitch, bool resize) {
	...
	if(tnum) {
		setuptexparameters(tnum, pixels, clamp, filter, format, target);
	}
    if(!pw) pw = w;
    if(!ph) ph = h;
    int tw = w, th = h;
    ...
    if(resize && pixels) {
		...
		if(mipmap) {
			component = compressedformat(component, tw, th);
		}
    }
...
void createcompressedtexture(int tnum, int w, int h, uchar *data, int align, int blocksize, int levels, int clamp, int filter, GLenum format, GLenum subtarget) {
	...
	if(tnum) {
		setuptexparameters(tnum, data, clamp, filter, format, target);
	}
...
static Texture *newtexture(Texture *t, const char *rname, ImageData &s, int clamp = 0, bool mipit = true, bool canreduce = false, bool transient = false, int compress = 0) {
    ...
    if(s.compressed) {
		...
		if(canreduce && texreduce) {
			loopi(min(texreduce, s.levels-1)) {
...
static bool texturedata(ImageData &d, const char *tname, Slot::Tex *tex = NULL, bool msg = true, int *compress = NULL) {
    ...
    if(!tname) {
        ...
        if(tex->name[0]=='<') {
            ...
            if(!file) { 
				if(msg) {
					conoutf(CON_ERROR, "could not load texture packages/%s", tex->name); 
				}
				return false; 
			}
            file++;
...
#ifdef LOADPVR
static bool texturepvrt(char *&fbuf, const char *tname, Slot::Tex *tex, bool msg, int *compress, size_t &size) {
    const char *cmds = NULL, *file = tname;
    if(!tname) {
        if(!tex) return false;
        if(tex->name[0]=='<') {
...
static Texture *newtexturepvrt(Texture *t, const char *rname, char *fbuf, int clamp = 0, bool mipit = true, bool canreduce = false, bool transient = false, int compress = 0) {
    if(!t) {
        char *key = newstring(rname);
        t = &textures[key];
        t->name = key;
	...
	t->bpp = 8;//Header.dwBitCount;
	//m_bUseAlpha = Header.dwAlphaBitMask > 0;
    return t;
}
#endif

void loadalphamask(Texture *t) {
	if (t->alphamask || t->bpp!=4 || 
		t->type&Texture::COMPRESSED) {
		return;
	}
    ImageData s;
    if(!texturedata(s, t->name, NULL, false) || 
		!s.data || s.bpp!=4 || s.compressed) {
		return;
	}
...
Texture *textureload(const char *name, int clamp, bool mipit, bool msg) {
    ...
    size_t fsize = 0;
	char *fbuf = NULL;
	if(texturepvrt(fbuf, tname, NULL, msg, &compress, fsize)) {
		if (fbuf != NULL) {
			return newtexturepvrt(NULL, tname, fbuf, clamp, mipit, false, false, compress);
		} else {
			return notexture;
		}
	}
    ...
    int compress = 0;
    ImageData s;
	if(texturedata(s, tname, NULL, msg, &compress)) {
...
void compactvslots(cube *c, int n) {
	if((compactvslotsprogress++&0xFFF)==0) {
		renderprogress(min(float(compactvslotsprogress)/allocnodes, 1.0f), 
			markingvslots ? "marking slots..." : "compacting slots...");
	}
    loopi(n) {
		if(c[i].children) {
...
void texture(char *type, char *name, int *rot, int *xoffset, int *yoffset, float *scale) {
	...
	loopi(sizeof(types)/sizeof(types[0])) {
    ...
    s.texmask |= 1<<tnum;
	if(s.sts.length()>=8) {
		conoutf(CON_WARN, "warning: too many textures in slot %d", slots.length()-1);
	}
            ...
            if(renderpath==R_FIXEDFUNCTION) {
				if(!ts.compressed) {
					loopv(s.sts) {
						...
						if(bs.w!=ts.w || bs.h!=ts.h) {
							scaleimage(bs, ts.w, ts.h);
						}
						switch(b.type) {
        ...
        case TEX_NORMAL:
			if(!ts.compressed) {
				loopv(s.sts) {
					...
					if(as.w!=ts.w || as.h!=ts.h) {
						scaleimage(as, ts.w, ts.h);
					}
					switch(a.type) {
...
static Slot &loadslot(Slot &s, bool forceload) {
    linkslotshader(s);
#ifndef SUPPORTTEXTURECOMBINE
    loopv(s.sts) {
        Slot::Tex &t = s.sts[i];
        if(t.combined >= 0) continue;
        switch(t.type) {
            case TEX_ENVMAP:
				if (hasCM && (renderpath != R_FIXEDFUNCTION || 
					(s.shader->type&SHADER_ENVMAP && s.ffenv && maxtmus >= 2) || forceload)) {
...
#else
	loopv(s.sts) {
		s.sts[i].t = textureload(s.sts[i].name, 0, false, false);
	}
#endif
    s.loaded = true;
    return s;
...
VSlot &lookupvslot(int index, bool load) {
    VSlot &s = vslots.inrange(index) && vslots[index]->slot ? 
		*vslots[index] : (slots.inrange(DEFAULT_GEOM) && 
		slots[DEFAULT_GEOM]->variants ? *slots[DEFAULT_GEOM]->variants : dummyvslot);
    if(load && !s.linked) {
		if(!s.slot->loaded) {
			loadslot(*s.slot, false);
		}
        linkvslotshader(s);
        s.linked = true;
    }
...
Texture *loadthumbnail(Slot &slot) {
	...
	if(vslot.colorscale == vec(1, 1, 1)) {
        ...
        defformatstring(prefix)("<thumbnail:%.2f/%.2f/%.2f>", 
			vslot.colorscale.x, vslot.colorscale.y, vslot.colorscale.z);
    ...
    if(slot.texmask&(1<<TEX_GLOW)) { 
        ...
        if(glow >= 0) {
            defformatstring(prefix)("<glow:%.2f/%.2f/%.2f>", 
				vslot.glowcolor.x, vslot.glowcolor.y, vslot.glowcolor.z); 
            addname(name, slot, slot.sts[glow], true, prefix);
        }
    }
    VSlot *layer = vslot.layer ? &lookupvslot(vslot.layer, false) : NULL;
    if(layer) {
		if(layer->colorscale == vec(1, 1, 1)) {
			addname(name, *layer->slot, layer->slot->sts[0], true, "<layer>");
		} else {
            defformatstring(prefix)("<layer:%.2f/%.2f/%.2f>", 
				vslot.colorscale.x, vslot.colorscale.y, vslot.colorscale.z);
	...
	if(t) {
        ...
        if(layer) {
			...
			if(layer->colorscale != vec(1, 1, 1)) {
				texmad(l, layer->colorscale, vec(0, 0, 0));
			}
        }
		if(!s.data) {
			t = slot.thumbnail = notexture;
		} else {
            int xs = s.w, ys = s.h;
			if(s.w > 64 || s.h > 64) {
				scaleimage(s, min(s.w, 64), min(s.h, 64));
			}
            if(g.data) {
				if(g.w != s.w || g.h != s.h) {
					scaleimage(g, s.w, s.h);
				}
...
void loadlayermasks() {
    loopv(slots) {
        ...
        if(slot.loaded && slot.layermaskname && !slot.layermask) {
			...
			if(!slot.layermask->data) {
				DELETEP(slot.layermask);
			}
        }
    }
}
...
Texture *cubemaploadwildcard(Texture *t, const char *name, bool mipit, bool msg, bool transient = false) {
    ...
    loopi(6) {
		...
		if(!format) {
			format = s.compressed ? s.compressed : texformat(s.bpp);
		} else 
		if ((s.compressed ? s.compressed : texformat(s.bpp))!=format || 
			(s.compressed && (s.w!=surface[0].w || s.h!=surface[0].h || 
			s.levels!=surface[0].levels))) {
			if(msg) {
				conoutf(CON_ERROR, "cubemap texture %s doesn't match other sides' format", sname);
			}
            return NULL;
        }
...
void clearenvmaps() {
    if(skyenvmap) {
		if(skyenvmap->type&Texture::TRANSIENT) {
			cleanuptexture(skyenvmap);
		}
        skyenvmap = NULL;
    }
	loopv(envmaps) {
		glDeleteTextures(1, &envmaps[i].tex);
	}
    envmaps.shrink(0);
}
...
GLuint lookupenvmap(ushort emid) {
	if (emid==EMID_SKY || emid==EMID_CUSTOM) {
		return skyenvmap ? skyenvmap->id : 0;
	}
    if (emid==EMID_NONE || 
		!envmaps.inrange(emid-EMID_RESERVED)) {
		return 0;
	}
...
bool reloadtexture(Texture &tex) {
    ...
    switch(tex.type&Texture::TYPE) {
        ...
        case Texture::IMAGE: {
            int compress = 0;
            ImageData s;
            if (!texturedata(s, tex.name, NULL, true, &compress) || 
        ...
        case Texture::CUBEMAP:
			if(!cubemaploadwildcard(&tex, NULL, tex.mipmap, true)) {
				return false;
			}
            break;
    }    
...
void reloadtex(char *name) {
    Texture *t = textures.access(path(name, true));
    if(!t) { 
		conoutf(CON_ERROR, "texture %s is not loaded", name); 
		return; 
	}
    if(t->type&Texture::TRANSIENT) { 
		conoutf(CON_ERROR, "can't reload transient texture %s", name); 
		return; 
	}
    DELETEA(t->alphamask);
    Texture oldtex = *t;
...
struct DDCOLORKEY { 
	uint dwColorSpaceLowValue, dwColorSpaceHighValue; 
};
...
void gendds(char *infile, char *outfile) {
#ifdef SUPPORTDDS
    if(!hasTC) { 
		conoutf(CON_ERROR, "OpenGL driver does not support texture compression"); 
		return; 
	}
    ...
    if(!compressed) { 
		conoutf(CON_ERROR, "failed compressing %s", infile); 
		return; 
	}
    int fourcc = 0;
    switch(format) {
...
void savepng(const char *filename, ImageData &image, bool flip) {
    ...
    if(!f) { 
		conoutf(CON_ERROR, "could not write to %s", filename); 
		return; 
	}
        ...
        for(int remaining = image.w; remaining > 0;) {
            ...
            if(compresstga) {
                ...
                for(uchar *scan = src; run < min(remaining, 128); run++) {
                    scan += image.bpp;
                    if (src[0]!=scan[0] || src[1]!=scan[1] || 
						src[2]!=scan[2] || (image.bpp==4 && src[3]!=scan[3])) break;
                }
                if(run > 1) {
...
void flipnormalmapy(char *destfile, char *normalfile) {
    ImageData ns;
...
void mergenormalmaps(char *heightfile, char *normalfile) {
    ImageData hs, ns;
    if (!loadimage(heightfile, hs) || 
		!loadimage(normalfile, ns) || 
		hs.w != ns.w || hs.h != ns.h) {
		return;
	}
    ImageData d(ns.w, ns.h, 3);
