...
bool hasVBO = false, hasDRE = false, hasOQ = false, hasTR = false, hasFBO = false, 
hasDS = false, hasTF = false, hasBE = false, hasBC = false, hasCM = false, 
hasNP2 = false, hasTC = false, hasTE = false, hasMT = false, hasD3 = false, 
hasAF = false, hasVP2 = false, hasVP3 = false, hasPP = false, hasMDA = false, 
hasTE3 = false, hasTE4 = false, hasVP = false, hasFP = false, hasGLSL = false, 
hasGM = false, hasNVFB = false, hasSGIDT = false, hasSGISH = false, hasDT = false, 
hasSH = false, hasNVPCF = false, hasRN = false, hasPBO = false, hasFBB = false, 
hasUBO = false, hasBUE = false, hasFC = false, hasTEX = false;
int hasstencil = 0;
...
//    int osversion = mac_osversion();  /* 0x1050 = 10.5 (Leopard) */
    ...
    if (strstr(exts, "GL_EXT_texture_env_combine") || 
        ...
        if (strstr(exts, "GL_EXT_texture_env_dot3") || 
			strstr(exts, "GL_ARB_texture_env_dot3")) {
			hasD3 = true;
		}
        if (dbgexts) conoutf(CON_INIT, "Using GL_ARB_texture_env_combine extension.");
	} else {
		conoutf(CON_WARN, "WARNING: No texture_env_combine extension! (your video card is WAY too old)");
	}
    if(strstr(exts, "GL_ARB_multitexture")) {
		...
		conoutf(CON_WARN, "WARNING: No multitexture extension!");
	}
#ifdef VBOENABLE
    if(strstr(exts, "GL_ARB_vertex_buffer_object"))  {
		...
		conoutf(CON_WARN, "WARNING: No vertex_buffer_object extension! (geometry heavy maps will be SLOW)");
	}
#endif
#ifdef __APPLE__
	...
	if (strstr(vendor, "Tungsten") || strstr(vendor, "Mesa") || 
		strstr(vendor, "DRI") || strstr(vendor, "Microsoft") || strstr(vendor, "S3 Graphics")) {
        avoidshaders = 1;
        maxtexsize = 256;
        reservevpparams = 20;
        batchlightmaps = 0;
    ...
    if (strstr(exts, "GL_ARB_shading_language_100") && 
		...
		strstr(exts, "GL_ARB_fragment_shader")) {
        ...
        if(checkglslsupport()) {
			...
			if(apple_glsldepth_bug) {
				conoutf(CON_WARN, "WARNING: Using Apple GLSL depth bug workaround. (use \"/apple_glsldepth_bug 0\" to disable if unnecessary");
			}
        }
    }
#endif
...
#ifdef WIN32
    if(strstr(exts, "GL_NV_vertex_program2_option")) { 
		usevp2 = 1; 
		hasVP2 = true; 
	}
    if(strstr(exts, "GL_NV_vertex_program3")) { 
		usevp3 = 1; 
		hasVP3 = true; 
	}
    if(strstr(exts, "GL_EXT_gpu_program_parameters")) {
        ...
        hasPP = true;
		if(dbgexts) {
			conoutf(CON_INIT, "Using GL_EXT_gpu_program_parameters extension.");
		}
    }
	...
	if(strstr(exts, "GL_EXT_bindable_uniform")) {
    ...
    if(strstr(exts, "GL_ARB_texture_cube_map")) {
		...
		conoutf(CON_WARN, "WARNING: No cube map texture support. (no reflective glass)");
	}
#endif
    extern int usenp2;
    if(strstr(exts, "GL_ARB_texture_non_power_of_two")) {
		...
		if(usenp2) conoutf(CON_WARN, "WARNING: Non-power-of-two textures not supported!");
	}
	...
	if(strstr(exts, "GL_SGIX_depth_texture")) {
        hasSGIDT = true;
...
void gl_init(int w, int h, int bpp, int depth, int fsaa)
{
    ...
    if(!useshaders || (useshaders<0 && avoidshaders) || !hasMT || !hasshaders) {
		if(!hasMT || !hasshaders) {
			conoutf(CON_WARN, "WARNING: No shader support! Using fixed-function fallback. (no fancy visuals for you)");
		} else 
		if(useshaders<0 && !hasTF) {
			conoutf(CON_WARN, "WARNING: Disabling shaders for extra performance. (use \"/shaders 1\" to enable shaders if desired)");
		}
        renderpath = R_FIXEDFUNCTION;
	} else {
		renderpath = hasGLSL ? (!hasVP || !hasFP || forceglsl > 0 ? R_GLSLANG : R_ASMGLSLANG) : R_ASMSHADER;
	}
...
void project(float fovy, float aspect, int farplane, bool flipx = false, bool flipy = false, 
			 bool swapxy = false, float zscale = 1) {
...
#else
    GLfloat ydist = nearplane * tan(fovy/2*RAD), xdist = ydist * aspect;
    glFrustumf(-xdist, xdist, -ydist, ydist, nearplane, farplane);
#endif
...
#else
    GLfloat ydist = nearplane * tan(curavatarfov/ 2 * RAD), xdist = ydist * aspect;
#endif
    vec4 scrpos;
...
#else
    glOrthof(-minimapradius.x, minimapradius.x, -minimapradius.y, minimapradius.y, 0, camera1->o.z + 1);
#endif
    glScalef(-1, 1, 1);
    glMatrixMode(GL_MODELVIEW);
...
void addmotionblur()
{
    ...
    if(!motiontex || motionw != screen->w || motionh != screen->h)
    {
        ...
        motionh = screen->h;
        lastmotion = 0;
        createtexture(motiontex, motionw, motionh, NULL, 3, 0, GL_RGB, GL_TEXTURE_RECTANGLE_ARB_);
    }

    glBindTexture(GL_TEXTURE_RECTANGLE_ARB_, motiontex);
    ...
    glDisable(GL_TEXTURE_2D);
    glEnable(GL_TEXTURE_RECTANGLE_ARB_);
    rectshader->set();
	...
	glDisableClientState(GL_TEXTURE_COORD_ARRAY);
#endif
    glDisable(GL_TEXTURE_RECTANGLE_ARB_);
    glEnable(GL_TEXTURE_2D);
    ...
    if(lastmillis - lastmotion >= motionblurmillis)
    {
        ...
        glCopyTexSubImage2D(GL_TEXTURE_RECTANGLE_ARB_, 0, 0, 0, 0, 0, screen->w, screen->h);
    }
#endif
}
...
void drawdamagescreen(int w, int h)
{
	...
	if(!damagetex) {
		damagetex = textureload("packages/hud/damage.png", 3);
	}
...
#define BUFFERRENDER glBlendFunc(GL_ONE, GL_ONE);\
			glDisableClientState(GL_NORMAL_ARRAY);\
			glDisableClientState(GL_COLOR_ARRAY);\
			glEnableClientState(GL_VERTEX_ARRAY);\
			glEnableClientState(GL_TEXTURE_COORD_ARRAY);\
...
void drawcrosshair(int w, int h, bool Cursor) {
	{
		...
		if(windowhit) {
			...
			if(!cursor) {
				cursor = textureload("data/guicursor.png", 3, true);
			}
			crosshair = cursor;
			chsize = cursorsize * w / SCREENW;
			g3d_cursorpos(cx, cy);
		} else { 
