...
#define BLURTILEMASK (0xFFFFFFFFU>>(32-BLURTILES))
    ...
    void setup(int w, int h)
    {
        ...
        target = texrect() ? GL_TEXTURE_RECTANGLE_ARB_ : GL_TEXTURE_2D;
    ...
    virtual void rendertiles()
    {
        ...
        if(target!=GL_TEXTURE_RECTANGLE_ARB_)
        {
            wscale /= texw;
            hscale /= texh;
        }
    ...
    void blur(int wantsblursize, float wantsblursigma, int x, int y, int w, int h, bool scissor)
    {
        ...
        loopi(2)
        {
            setblurshader(i, target == GL_TEXTURE_RECTANGLE_ARB_ ? 1 : (i ? texh : texw), blursize, blurweights, bluroffsets, target);
    ...
    void render(int w, int h, int blursize = 0, float blursigma = 0)
    {
        ...
        if(w!=texw || h!=texh || (texrect() ? target!=GL_TEXTURE_RECTANGLE_ARB_ : target!=GL_TEXTURE_2D) || (hasFBO && (swaptexs() && !rtsharefb ? !blurfb : blurfb))) cleanup();
    ...
    void debug()
    {
        ...
        (target==GL_TEXTURE_RECTANGLE_ARB_ ? rectshader : defaultshader)->set();
        glColor3f(1, 1, 1);
        glEnable(target);
        glBindTexture(target, rendertex);
