...
struct gui : g3d_gui {
    ...
    bool visible() { 
		return (!tcurrent || tpos==*tcurrent) && !layoutpass; 
	}
    ...
    int text  (const char *text, int color, const char *icon) { 
		autotab(); 
		return button_(text, color, icon, false, false); 
	}
    
	int button(const char *text, int color, const char *icon) { 
		autotab(); 
		return button_(text, color, icon, true, false); 
	}
    
	int title (const char *text, int color, const char *icon) { 
		autotab(); 
		return button_(text, color, icon, false, true); 
	}
	...
	void progress(float percent) { 
		autotab(); 
		line_(FONTH * 2 / 5, percent); 
	}

    //use to set min size (useful when you have progress bars)
    void strut(float size) { 
		layout(isvertical() ? int(size * FONTW) : 0, isvertical() ? 0 : int(size * FONTH)); 
	}
    
	//add space between list items
    void space(float size) { 
		layout(isvertical() ? 0 : int(size * FONTW), isvertical() ? int(size * FONTH) : 0); 
	}
...
#else
            if(editing) glColor4f(1, 0, 0, 1);
            else glColor4f((color>>16) / 255.0f, ((color>>8)&0xFF) / 255.0f, (color&0xFF) / 255.0f, 1.0f);
#endif
    ...
    void icon_(Texture *t, bool overlaid, int x, int y, int size, bool hit)
    {
        ...
        glEnd();
#else
        static const float tc[4][2] = { { 0, 0 }, { 1, 0 }, { 0, 1 }, { 1, 1 } };
		GLfloat verts[] = {
			x, y,
			x+xs, y,
			x, y+ys,
        ...
        {
			if(!overlaytex) {
				overlaytex = textureload("data/guioverlay.png", 3);
			}
            glBindTexture(GL_TEXTURE_2D, overlaytex->id);
#ifdef WIN32
            glColor3fv(light.v);
#else
			glColor4f(light.x, light.y, light.z, 1.0f);
#endif
            rect_(x, y, xs, ys, 0);
        }
    }        
...
#else
        if(slot.loaded) glColor4f(color.x*vslot.colorscale.x, color.y*vslot.colorscale.y, color.z*vslot.colorscale.z, 1.0f);
		else glColor4f(color.x, color.y, color.z, 1.0f);
#endif
        glBindTexture(GL_TEXTURE_2D, t->id);
#ifdef ORIGINAL
        glBegin(GL_TRIANGLE_STRIP);
...
#else
            if(hit || overlaid) glColor4f(color.x*vslot.glowcolor.x, color.y*vslot.glowcolor.y, color.z*vslot.glowcolor.z, 1.0f);
            else glColor4f(vslot.glowcolor.x, vslot.glowcolor.y, vslot.glowcolor.z, 1.0f);
#endif
#ifdef ORIGINAL
            glBegin(GL_TRIANGLE_STRIP);
            glTexCoord2fv(tc[0]); glVertex2f(x,    y);
...
#else
            glColor4f(color.x*layer->colorscale.x, color.y*layer->colorscale.y, color.z*layer->colorscale.z, 1.0f);
#endif
#ifdef ORIGINAL
            glBegin(GL_TRIANGLE_STRIP);
    ...
    {		
        ...
        {
			if(!slidertex) {
				slidertex = textureload("data/guislider.png", 3);
			}
            glEnable(GL_TEXTURE_2D);
    ...
    {
		if(!skintex) {
			skintex = textureload("data/guiskin.png", 3);
		}
        ...
        {	
            ...
            loopi(n)
            {
				...
				while(ystep > 0) 
                {
                    ...
                    while(xstep > 0) 
                    {
...
#else
						/*BufferVerts[BufferSize] = xo;
						BufferVerts[BufferSize + 1] = yo;
						BufferVerts[BufferSize + 2] = xo + xstep;
						BufferVerts[BufferSize + 3] = yo;
						BufferVerts[BufferSize + 4] = xo + xstep;
						BufferVerts[BufferSize + 5] = yo + ystep;
						BufferVerts[BufferSize + 6] = xo;
						BufferVerts[BufferSize + 7] = yo + ystep;
						BufferTexts[BufferSize] = tleft;
						BufferTexts[BufferSize + 1] = ttop; 
						BufferTexts[BufferSize + 2] = tright; 
						BufferTexts[BufferSize + 3] = ttop; 
						BufferTexts[BufferSize + 4] = tright; 
						BufferTexts[BufferSize + 5] = tbottom; 
						BufferTexts[BufferSize + 6] = tleft; 
						BufferTexts[BufferSize + 7] = tbottom; 
						BufferSize += 8;*/
						BufferVerts[BufferSize] = xo;
						BufferVerts[BufferSize + 1] = yo;
						BufferVerts[BufferSize + 2] = xo + xstep;
						...
						BufferVerts[BufferSize + 5] = yo + ystep;
						BufferVerts[BufferSize + 6] = xo;
						BufferVerts[BufferSize + 7] = yo;
						BufferVerts[BufferSize + 8] = xo + xstep;
						BufferVerts[BufferSize + 9] = yo + ystep;
						BufferVerts[BufferSize + 10] = xo;
						BufferVerts[BufferSize + 11] = yo + ystep;
						BufferTexts[BufferSize] = tleft;
						BufferTexts[BufferSize + 1] = ttop; 
						BufferTexts[BufferSize + 2] = tright; 
						BufferTexts[BufferSize + 3] = ttop; 
						...
						BufferTexts[BufferSize + 5] = tbottom; 
						BufferTexts[BufferSize + 6] = tleft;
						BufferTexts[BufferSize + 7] = ttop; 
						BufferTexts[BufferSize + 8] = tright; 
						BufferTexts[BufferSize + 9] = tbottom; 
						BufferTexts[BufferSize + 10] = tleft; 
						BufferTexts[BufferSize + 11] = tbottom; 
						BufferSize += 12;
#endif
                        xtraverts += 4;
			...
			if(quads) {
				...
				glEnableClientState(GL_VERTEX_ARRAY);
				glEnableClientState(GL_TEXTURE_COORD_ARRAY);
				glVertexPointer(2, GL_FLOAT, 0, BufferVerts);
				glTexCoordPointer(2, GL_FLOAT, 0, BufferTexts);
				glDrawArrays(GL_TRIANGLES, 0, BufferSize / 2);
				glDisableClientState(GL_NORMAL_ARRAY);
				glDisableClientState(GL_COLOR_ARRAY);
				glDisableClientState(GL_VERTEX_ARRAY);
				glDisableClientState(GL_TEXTURE_COORD_ARRAY);
				...
				break; //if it didn't happen on the first pass, it won't happen on the second..
			}
        }
    ...
    void end()
    {
        ...
        {	
            ...
            if(!windowhit && !passthrough) {
                ...
                if ((mousebuttons & G3D_PRESSED) && 
					(fabs(hitx-firstx) > 2 || fabs(hity - firsty) > 2)) {
					mousebuttons |= G3D_DRAGGED;
				}
                if(dist>=0 && hitx>=-xsize/2 && hitx<=xsize/2 && hity<=0) {
                    if (hity>=-ysize || 
						(tcurrent && hity>=-ysize-(FONTH-2*INSERT)-((skiny[6]-skiny[1])-(skiny[3]-skiny[2]))*SKIN_SCALE && 
						hitx<=tx-xsize/2)) {
                        windowhit = this;
					}
                }
			...
			if(tcurrent && tx<xsize) {
				drawskin(
					curx+tx-skinx[5]*SKIN_SCALE, -ysize-skiny[6]*SKIN_SCALE, xsize-tx, 
					FONTH, 9, 1, gui2d ? 1 : 2, light, alpha);
			}
            glPopMatrix();
        }
        poplist();
...
bool menukey(int code, bool isdown, int cooked, int mouseID) {
...
bool g3d_movecursor(int dx, int dy) {
    ...
    cursorx = dx / (1.0f * screen->w);
    cursory = dy / (1.0f * screen->h);
