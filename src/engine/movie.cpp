        ...
        if(usefbo)
        {
            ...
            if(tw != scalew || th != scaleh)
            {
                ...
                loopi(2)
                {
                    ...
                    createtexture(scaletex[i], tw, th, NULL, 3, 1, GL_RGB, GL_TEXTURE_RECTANGLE_ARB_);
                }
                scalew = tw;
                scaleh = th;
            }
            ...
            if(tw < (uint)screen->w || th < (uint)screen->h)
            {
                ...
                glFramebufferTexture2D_(GL_DRAW_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_RECTANGLE_ARB_, scaletex[0], 0);
            ...
            {
                glBindTexture(GL_TEXTURE_RECTANGLE_ARB_, scaletex[0]);
            ...
            if(tw > m.w || th > m.h || (!accelyuv && renderpath != R_FIXEDFUNCTION && tw >= m.w && th >= m.h))
            {
                ...
                do
                {
                    glFramebufferTexture2D_(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_RECTANGLE_ARB_, scaletex[1], 0);
                    glBindTexture(GL_TEXTURE_RECTANGLE_ARB_, scaletex[0]);
