...
#else
        if(hmapedit==1)
            glColor4f(0 / 255.0f, hmapsel ? 255 / 255.0f : 40 / 255.0f, 0, 1.0f);
        else
            glColor4f(120 / 255.0f,120 / 255.0f,120 / 255.0f, 1.0f);
#endif
...
#else
        if(hmapedit==1)         // 3D selection box
            glColor4f(0,120 / 255.0f,0, 1.0f);
        else 
            glColor4f(0,0,120 / 255.0f, 1.0f);
#endif
            ...
            loop(h, texguiheight) 
            {
                ...
                loop(w, texguiwidth) 
                {
                    ...
                    if(ti<slots.length()) 
                    {
                        ...
                        if (g.texture(vslot, 1.0f, true) & G3D_UP && 
							(slot.loaded || slot.thumbnail!=notexture)) {
                            edittex(vslot.index);
						}
                    } else {
                        g.texture(dummyvslot, 1.0, false); //create an empty space
