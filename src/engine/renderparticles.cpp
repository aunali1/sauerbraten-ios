...
struct meterrenderer : listrenderer
{
    ...
    void renderpart(listparticle *p, const vec &o, const vec &d, int blend, int ts, uchar *color)
    {
...
#else
        if(basetype==PT_METERVS) glColor4f(p->color2[0], p->color2[1], p->color2[2], 1);
        ...
        glColor3ubv(color);
#else
		glColor4f(color[0]/255.0f,color[1]/255.0f,color[2]/255.0f,1);
#endif
#ifdef ORIGINAL
        glBegin(GL_TRIANGLE_STRIP);
        loopk(10)
...
static inline void genpos(const vec &o, const vec &d, float size, int grav, int ts, partvert *vs)
{
    ...
    vs[3].pos = vec(o.x + udir.x, o.y + udir.y, o.z + udir.z);
    vs[4].pos = vec(o.x - udir.x, o.y - udir.y, o.z - udir.z);
    vs[5].pos = vec(o.x - vdir.x, o.y - vdir.y, o.z - vdir.z);
}
...
inline void genpos<PT_TAPE>(const vec &o, const vec &d, float size, int ts, int grav, partvert *vs)
{
    ...
    vs[4].pos = vec(o.x+c.x, o.y+c.y, o.z+c.z);
    vs[5].pos = vec(d.x+c.x, d.y+c.y, d.z+c.z);
}
...
inline void genrotpos<PT_PART>(const vec &o, const vec &d, float size, int grav, int ts, partvert *vs, int rot)
{
    ...
    (vs[3].pos = o).add(vec(camright).mul(coeffs[0].x*size)).add(vec(camup).mul(coeffs[0].y*size));
    (vs[4].pos = o).add(vec(camright).mul(coeffs[2].x*size)).add(vec(camup).mul(coeffs[2].y*size));
    (vs[5].pos = o).add(vec(camright).mul(coeffs[3].x*size)).add(vec(camup).mul(coeffs[3].y*size));
}
...
//#ifdef SUPPORTDRAWQUAD
        ...
        glDrawArrays(GL_TRIANGLES, 0, numparts * 6);
//#endif
    }
};
...
#else
        glOrthof(0, FONTH*n*2*screen->w/float(screen->h), FONTH*n*2, 0, -1, 1); //squeeze into top-left corner        
#endif
    ...
    loopi(sizeof(parts)/sizeof(parts[0]))
    {
        ...
        if(changedbits != 0x0000)
        {
            ...
            if(changedbits&PT_SOFT)
            {
                if(flags&PT_SOFT)
                {
                    if(depthfxtex.target==GL_TEXTURE_RECTANGLE_ARB_)
                    {
