...
struct lightningrenderer : listrenderer {
    ...
    void renderpart(listparticle *p, const vec &o, const vec &d, int blend, int ts, uchar *color) {
		...
		if (type & PT_MOD) { 
			//multiply alpha into color
            glColor4f(((color[0]*blend)>>8)/255.0f, ((color[1]*blend)>>8)/255.0f, ((color[2]*blend)>>8)/255.0f, 1.0f);
		} else {
            glColor4f(color[0]/255.0f, color[1]/255.0f, color[2]/255.0f, blend/255.0f);
		}
#endif
