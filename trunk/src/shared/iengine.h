...
static inline bool insideworld(const ivec &o) {
	extern int worldsize;
    return uint(o.x) < uint(worldsize) && 
		uint(o.y) < uint(worldsize) && 
		uint(o.z) < uint(worldsize);
}
...
extern void filtertext(char *dst, const char *src, bool whitespace = true, int len = sizeof(safe_string)-1);
