	...
	const float		fSplitDifference)
{
	...
	struct SVtxData
	{
		...
		PVRTVECTOR3f	pvBin[cnMaxSharedVtx];		// Binormal (one per triangle referencing this vtx)
	...
	*pVtxOut		= (char*)malloc(MAX_VERTEX_OUT * nStride);
	...
	for(nCurr = 0; nCurr < nTriNum; ++nCurr) {
		...
		PVRTDataTypeRead((PVRTVECTOR4f*)pfPos0, (char*)&pVtx[nIdx0 * nStride] + nOffsetPos, eTypePos, 3);
		PVRTDataTypeRead((PVRTVECTOR4f*)pfPos1, (char*)&pVtx[nIdx1 * nStride] + nOffsetPos, eTypePos, 3);
		PVRTDataTypeRead((PVRTVECTOR4f*)pfPos2, (char*)&pVtx[nIdx2 * nStride] + nOffsetPos, eTypePos, 3);

		PVRTDataTypeRead((PVRTVECTOR4f*)pfNor0, (char*)&pVtx[nIdx0 * nStride] + nOffsetNor, eTypeNor, 3);
		PVRTDataTypeRead((PVRTVECTOR4f*)pfNor1, (char*)&pVtx[nIdx1 * nStride] + nOffsetNor, eTypeNor, 3);
		PVRTDataTypeRead((PVRTVECTOR4f*)pfNor2, (char*)&pVtx[nIdx2 * nStride] + nOffsetNor, eTypeNor, 3);

		PVRTDataTypeRead((PVRTVECTOR4f*)pfTex0, (char*)&pVtx[nIdx0 * nStride] + nOffsetTex, eTypeTex, 3);
		PVRTDataTypeRead((PVRTVECTOR4f*)pfTex1, (char*)&pVtx[nIdx1 * nStride] + nOffsetTex, eTypeTex, 3);
		PVRTDataTypeRead((PVRTVECTOR4f*)pfTex2, (char*)&pVtx[nIdx2 * nStride] + nOffsetTex, eTypeTex, 3);
	...
	for(nVert = 0; nVert < nVtxNum; ++nVert) {
		...
		for(nCurr = 0; nCurr < nTSpassLen; ++nCurr) {
			...
			PVRTMatrixVec3NormalizeF(*(PVRTVECTOR3f*)pfPos0, *(PVRTVECTOR3f*)pfPos0);
			PVRTMatrixVec3NormalizeF(*(PVRTVECTOR3f*)pfPos1, *(PVRTVECTOR3f*)pfPos1);
			...
			PVRTDataTypeWrite((char*)&(*pVtxOut)[(*pnVtxNumOut) * nStride] + nOffsetTan, eTypeTan, 3, (PVRTVECTOR4f*)pfPos0);
			PVRTDataTypeWrite((char*)&(*pVtxOut)[(*pnVtxNumOut) * nStride] + nOffsetBin, eTypeBin, 3, (PVRTVECTOR4f*)pfPos1);
