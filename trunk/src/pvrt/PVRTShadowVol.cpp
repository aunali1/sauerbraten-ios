...
******************************************************************************/
#if !defined(_UITRON_) && !defined(__SYMBIAN32__)
//#include <malloc.h>
#endif
...
** Code
****************************************************************************/
static unsigned short FindOrCreateVertex(PVRTSvShadowMesh * const psMesh, const PVRTVECTOR3 * const pV) {
...
static PVRTSvMEdge *FindOrCreateEdge(PVRTSvShadowMesh * const psMesh, const PVRTVECTOR3 * const pv0, const PVRTVECTOR3 * const pv1) {
	unsigned int	nCurr;
	...
	const unsigned int		nNumFaces)
{
	...
	*/
	...
	psMesh->pE = (PVRTSvMEdge*)malloc(nNumFaces * sizeof(*psMesh->pE) * 3);
	psMesh->pT = (PVRTSvMTriangle*)malloc(nNumFaces * sizeof(*psMesh->pT));
	_ASSERT(psMesh->pV);
	_ASSERT(psMesh->pE);
	_ASSERT(psMesh->pT);
	...
	*/
	...
	psMesh->pE = (PVRTSvMEdge*)realloc(psMesh->pE, psMesh->nE * sizeof(*psMesh->pE));
	psMesh->pT = (PVRTSvMTriangle*)realloc(psMesh->pT, psMesh->nT * sizeof(*psMesh->pT));
	_ASSERT(psMesh->pV);
	_ASSERT(psMesh->pE);
	_ASSERT(psMesh->pT);
...
{
	PVRTSvSilhouetteProjectedBuild(psVol, dwVisFlags,psMesh, (PVRTVECTOR3*) pvLightModel, bPointLight);
}
	...
	const float				fVolLength)
{
	...
	*/
	PVRTTransVec3TransformArray(&BoundingHyperCubeT[0], sizeof(*BoundingHyperCubeT), pBoundingBox->Point,	sizeof(*pBoundingBox->Point),	pmTrans, 8);
	...
	*/
	PVRTSvBoundingBoxExtrude(pvExtrudedCube, pBoundingBox, pvLightMdl, bPointLight, fVolLength);

	/*
		Transform to projection space
	*/
	PVRTTransVec3TransformArray(&BoundingHyperCubeT[8], sizeof(*BoundingHyperCubeT), pvExtrudedCube, sizeof(*pvExtrudedCube), pmTrans, 8);
