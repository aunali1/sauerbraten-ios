...
void keypress(int code, bool isdown, int cooked, int mouseID) {
	...
	if(!menukey(code, isdown, cooked, mouseID)) {
		...
		if (code >= 0) {
			...
			cx = cx * SCREENW;
			cy = cy * SCREENH;
			static bool MovementTouch = false;
			static bool SwapTouch = false;
			...
			if (cx > TOUCHFIREX - TOUCHSIZE && 
				cx < TOUCHFIREX + TOUCHSIZE && 
				cy > TOUCHFIREY - TOUCHSIZE && 
				cy < TOUCHFIREY + TOUCHSIZE) {
				FireTouch = true;
				FireTouchX = cx;
				FireTouchY = cy;
