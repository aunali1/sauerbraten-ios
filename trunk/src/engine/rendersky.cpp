...
void loadsky(const char *basename, Texture *texs[6]) {
    ...
    loopi(6) {
        ...
        if((texs[i] = textureload(name, 3, true, false))==notexture) {
			...
			if((texs[i] = textureload(name, 3, true, false))==notexture) {
				conoutf(CON_ERROR, "could not load sky texture packages/%s_%s", basename, side);
			}
        }
    }
#else
...
#else
	GLfloat verts[] = {
		x3, y3, z3, x2, y2, z2, x0, y0, z0, x1, y1, z1
	};
	GLfloat texcoords[] = {
		s3, t3, s2, t2, s0, t0, s1, t1
	};
...
static void drawdome()
{
    ...
    if(hasDRE) {
		...
		glDrawElements(GL_TRIANGLES, domenumindices + fogdomecap*domecapindices, GL_UNSIGNED_SHORT, domeindices);
	}
...
#else
    if(clampsky) glDepthRangef(1, 1);
    glColor4f((skyboxcolour>>16)/255.0f, ((skyboxcolour>>8)&255)/255.0f, (skyboxcolour&255)/255.0f,1);
#endif
    glPushMatrix();
    glLoadMatrixf(viewmatrix.v);
    glRotatef(camera1->roll, 0, 1, 0);
