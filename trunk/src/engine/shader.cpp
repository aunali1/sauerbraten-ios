...
static void linkslotshaderparams(vector<ShaderParam> &params, Shader *sh, bool load) {
    if(sh) loopv(params) {
		...
		if(load) loopv(params) params[i].loc = -1;
	}
}

void linkslotshader(Slot &s, bool load) {
    if(!s.shader) return;
	if(load && !s.shader->detailshader) {
		s.shader->fixdetailshader();
	}
...
void addshaderparam(const char *name, int type, int n, float x, float y, float z, float w) {
    if ((type==SHPARAM_VERTEX || type==SHPARAM_PIXEL) && 
		(n<0 || n>=MAXSHADERPARAMS)) {
...
static int allocatepostfxtex(int scale) {
    ...
    createtexture(t.id, max(screen->w>>scale, 1), max(screen->h>>scale, 1), NULL, 3, 1, GL_RGB, GL_TEXTURE_RECTANGLE_ARB_);
...
void renderpostfx()
{
    ...
    glBindTexture(GL_TEXTURE_RECTANGLE_ARB_, postfxtexs[binds[0]].id);
    ...
    loopv(postfxpasses)
    {
        ...
        else
        {
			...
			if(hasFBO) {
				glFramebufferTexture2D_(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_RECTANGLE_ARB_, postfxtexs[tex].id, 0);
			}
        }
        ...
        loopj(NUMPOSTFXBINDS) if(p.inputs&(1<<j) && binds[j] >= 0) {
            ...
            glBindTexture(GL_TEXTURE_RECTANGLE_ARB_, postfxtexs[binds[j]].id);
            ++tmu;
        }
		if(tmu) {
        ...
        if(tex >= 0)
        {
            ...
            if(!hasFBO) {
                glBindTexture(GL_TEXTURE_RECTANGLE_ARB_, postfxtexs[tex].id);
                glCopyTexSubImage2D(GL_TEXTURE_RECTANGLE_ARB_, 0, 0, 0, 0, 0, w, h);
            }
        }
    }
}
...
void setblurshader(int pass, int size, int radius, float *weights, float *offsets, GLenum target)
{
    ...
    Shader *&s = (target == GL_TEXTURE_RECTANGLE_ARB_ ? blurrectshader : blurshader)[radius-1][pass];
    if(!s)
    {
        defformatstring(name)("blur%c%d%s", 'x'+pass, radius, target == GL_TEXTURE_RECTANGLE_ARB_ ? "rect" : "");
