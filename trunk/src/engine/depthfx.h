...
static struct depthfxtexture : rendertarget {
    const GLenum *colorformats() const {
        static const GLenum colorfmts[] = { 
#ifdef WIN32
			GL_FLOAT_RG16_NV, GL_RGB16F_ARB, GL_RGB16, GL_RGBA, GL_RGBA8, GL_RGB, GL_RGB8, GL_FALSE 
#else
			GL_RGBA, GL_RGBA8, GL_RGB, GL_RGB8, GL_FALSE 
#endif
		};
    ...
    bool screenview() const { 
		return depthfxrect!=0; 
	}
    
	bool texrect() const { 
		return depthfxrect && hasTR; 
	}
    
	bool filter() const { 
		return depthfxfilter!=0; 
	}
    
	bool highprecision() const { 
#ifdef WIN32
		return colorfmt==GL_FLOAT_RG16_NV || colorfmt==GL_RGB16F_ARB || colorfmt==GL_RGB16; 
#else
		return false;
#endif
	}
    bool emulatehighprecision() const { 
		return depthfxemuprecision && !depthfxfilter; 
	}
...
bool binddepthfxtex() {
    if (renderpath!=R_FIXEDFUNCTION && !reflecting && !refracting && 
		depthfx && depthfxtex.rendertex && numdepthfxranges>0) {
    ...
    if (renderpath!=R_FIXEDFUNCTION && !reflecting && !refracting && 
        ...
        if(!depthfxtex.highprecision()) {
            ...
            if(!depthfxtex.emulatehighprecision()) {
				loopi(numdepthfxranges) {
					if(depthfxowners[i]==owner) {
...
void drawdepthfxtex() {
	if(!depthfx || renderpath==R_FIXEDFUNCTION) {
		return;
	}
