...
struct QuadNode
{
    ...
    void genmatsurfs(uchar mat, uchar orient, uchar flags, int z, materialsurface *&matbuf) {
		if (filled == 0xF) {
			genmatsurf(mat, orient, flags, x, y, z, size, matbuf);
		} else 
		if(filled) {
            int csize = size>>1;
			loopi(4) {
		...
		loopi(4) {
			if(child[i]) {
				child[i]->genmatsurfs(mat, orient, flags, z, matbuf);
			}
		}
    }
};
...
#else
                    case MAT_WATER:    glColor4f(255/255.0f, 128/255.0f,   0, 1); break; // blue
                    case MAT_CLIP:     glColor4f(  0, 255/255.0f, 255/255.0f, 1); break; // red
                    case MAT_GLASS:    glColor4f(255/255.0f,   0,   0, 1); break; // cyan
                    case MAT_NOCLIP:   glColor4f(255/255.0f,   0, 255/255.0f, 1); break; // green
                    case MAT_LAVA:     glColor4f(  0, 128/255.0f, 255/255.0f, 1); break; // orange
                    case MAT_GAMECLIP: glColor4f(  0,   0, 255/255.0f, 1); break; // yellow
                    case MAT_DEATH:    glColor4f(192/255.0f, 192/255.0f, 192/255.0f, 1); break; // black
                    case MAT_ALPHA:    glColor4f(  0, 255/255.0f,   0, 1); break; // pink
#endif
                    default: continue;
                }
                lastmat = m.material;
                ...
                case MAT_WATER:
                    ...
                    if(lastmat!=MAT_WATER || (lastorient==O_TOP)!=(m.orient==O_TOP))
                    {
						...
						if (renderpath==R_FIXEDFUNCTION || 
							((!waterfallrefract || reflecting || refracting) && (!hasCM || !waterfallenv))) {
                            glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_COLOR);
#ifdef WIN32							
                            glColor3ubv(wfcol);
#else
							glColor4f(wfcol[0], wfcol[1], wfcol[2], wfcol[3]);
#endif
                            foggedshader->set();
                            fogtype = 0;
                            if(!blended) { glEnable(GL_BLEND); blended = true; }
...
#ifdef SUPPORTCUBEMAP
						|| (envmapped==m.envmap && textured==GL_TEXTURE_CUBE_MAP_ARB)
#endif
						) && lastmat==MAT_GLASS) break;
                    xtraverts += varray::end();
                    if(m.envmap!=EMID_NONE && glassenv)
                    {
#ifdef SUPPORTCUBEMAP
...
#else
                            glColor4f(0.3f, 0.15f, 0.0f, 1.0f);
#endif
                            foggednotextureshader->set();
                            fogtype = 0;
                        }
