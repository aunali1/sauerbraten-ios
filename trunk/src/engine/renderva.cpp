...
int isvisiblesphere(float rad, const vec &cv) {
	...
	//VFC_NOT_VISIBLE;    
	// culling when fog is closer than size of world results in HOM
	if(dist > -rad) {
		v = VFC_PART_VISIBLE;
	}
    return v;
...
void drawbb(const ivec &bo, const ivec &br, const vec &camera) {
	...
	loopi(6) {
        int dim = dimension(i), coord = dimcoord(i);
        if(coord) {
			if(camera[dim] < bo[dim] + br[dim]) {
				continue;
			}
        } else 
		if(camera[dim] > bo[dim]) {
			continue;
		}
        loopj(4) {
            const ivec &cc = cubecoords[fv[i][j]];
			BufferVerts[BufferSize] = cc.x ? (float)bo.x + br.x : (float)bo.x;
            BufferVerts[BufferSize + 1] = cc.y ? (float)bo.y + br.y : (float)bo.y;
			BufferVerts[BufferSize + 2] = cc.z ? (float)bo.z + br.z : (float)bo.z;
			BufferSize += 3;
        }
        xtraverts += 4;
    }
	...
	glVertexPointer(3, GL_FLOAT, 0, BufferVerts);
	glDrawArrays(GL_TRIANGLES, 0, BufferSize / 3);
	glVertexPointer(3, GL_FLOAT, VTXSIZE, LastPointer);
#endif
}
...
#else
    glColor4f(((outline>>16)&0xFF)/255.0f, ((outline>>8)&0xFF)/255.0f, (outline&0xFF)/255.0f, 1.0f);
#endif	
#ifdef WIN32
...
static void changevbuf(renderstate &cur, int pass, vtxarray *va)
{
    ...
    glVertexPointer(3, GL_FLOAT, VTXSIZE, va->vdata[0].pos.v);
	LastPointer = (GLvoid*)va->vdata[0].pos.v;
    if(pass==RENDERPASS_LIGHTMAP)
    {
		...
		if(cur.glowtmu >= 0 && hasCM && maxtmus >= 2) {
			glNormalPointer(GL_BYTE, VTXSIZE, va->vdata[0].norm.v);
		}
    }
...
static void changeslottmus(renderstate &cur, int pass, Slot &slot, VSlot &vslot)
{
    ...
    if(renderpath==R_FIXEDFUNCTION)
    {
        ...
        else if(pass==RENDERPASS_DYNLIGHT)
        {
            ...
            { 
                cur.colorscale = vslot.colorscale; 
                glColor4f(cur.lightcolor.x*vslot.colorscale.x, cur.lightcolor.y*vslot.colorscale.y, cur.lightcolor.z*vslot.colorscale.z,1);
            } 
        } 
        if((pass==RENDERPASS_LIGHTMAP || pass==RENDERPASS_ENVMAP) && 
			slot.shader->type&SHADER_ENVMAP && slot.ffenv && hasCM && maxtmus >= 2 && envpass) {
		...
		if(slot.texmask&(1<<TEX_GLOW)) {
			...
			if(pass==RENDERPASS_GLOW && vslot.skipped&(1<<TEX_GLOW)) {
				changeglow(cur, pass, slot, vslot);
			}
        } else 
		if(cur.mtglow) {
			goto noglow;
...
static void renderbatches(renderstate &cur, int pass)
{
    ...
    if(curbatch >= 0)
    {
        ...
        if(!cur.colormask) { 
			cur.colormask = true; 
			glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, cur.alphaing ? GL_FALSE : GL_TRUE); 
		}
    }        
    while(curbatch >= 0)
    {
        ...
        if(cur.vslot != &b.vslot) 
        {
            ...
            if (cur.texgendim != b.es.dim || 
				(cur.texgendim <= 2 && cur.texgenvslot != &b.vslot) || 
				(!cur.mttexgen && cur.mtglow && !cur.envscale.x)) {
				changetexgen(cur, b.es.dim, *b.vslot.slot, b.vslot);
			}
        }
...
#else
	glColor4f(watercolor.x, watercolor.y, watercolor.z, 1);
#endif
    loopv(foggedvas)
    {
...
void rendergeom(float causticspass, bool fogpass) {
    ...
    for(vtxarray *va = FIRSTVA; va; va = NEXTVA) {
        if(!va->texs) continue;
        if(refracting) {
            if((refracting < 0 ? va->geommin.z > reflectz : va->geommax.z <= reflectz) || 
				va->occluded >= OCCLUDE_GEOM) continue;
            if(ishiddencube(va->o, va->size)) continue;
        } else 
		if(reflecting) {
            if(va->geommax.z <= reflectz) continue;
		...
		if(doOQ && (zpass || va->distance > oqdist) && !insideva(va, camera1->o)) {
            ...
            va->occluded = va->query && va->query->owner == va && checkquery(va->query) ? 
				min(va->occluded+1, int(OCCLUDE_BB)) : OCCLUDE_NOTHING;
            ...
            if(va->occluded >= OCCLUDE_GEOM) {
                if(va->query) {
					if(!zpass && geombatches.length()) {
						glEnableClientState(GL_VERTEX_ARRAY);
						renderbatches(cur, nolights ? RENDERPASS_COLOR : RENDERPASS_LIGHTMAP);
					}
    ...
    if(blends && (renderpath!=R_FIXEDFUNCTION || !nolights))
    {
		...
		if(geombatches.length()) {
			glEnableClientState(GL_VERTEX_ARRAY);
			renderbatches(cur, RENDERPASS_LIGHTMAP);
		}
        cur.blending = false;
        glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE);
        glDisable(GL_BLEND);
    ...
    if(renderpath==R_FIXEDFUNCTION ? (glowpass && cur.skipped) || (causticspass>=1 && cur.causticstmu<0) || (shadowmap && shadowmapcasters) || hasdynlights : causticspass)
    {
        ...
        if(renderpath==R_FIXEDFUNCTION && shadowmap && shadowmapcasters)
        {
            ...
            glFogfv(GL_FOG_COLOR, zerofog);
            pushshadowmap();
            if(dbgffsm) { glDisable(GL_BLEND); glDisable(GL_TEXTURE_2D); glColor4f(1, 0, 1, 1); }
        ...
        if(renderpath==R_FIXEDFUNCTION && hasdynlights)
        {
            ...
            for(int n = 0; getdynlight(n, cur.dynlightpos, cur.dynlightradius, cur.lightcolor); n++)
            {
                ...
                glColor4f(cur.lightcolor.x, cur.lightcolor.y, cur.lightcolor.z, 1);
                if(ffdlscissor) {
    ...
    loop(front, 2) if(front || hasback)
    {
        ...
        if(renderpath==R_FIXEDFUNCTION && glowpass && cur.skipped)
        {
            ...
            if(cur.skipped&(1<<TEX_ENVMAP))
            {
				...
				if(geombatches.length()) {
					renderbatches(cur, RENDERPASS_ENVMAP);
					glEnableClientState(GL_VERTEX_ARRAY);
				}
                cleanupenvpass(cur);
            }
            if(cur.skipped&(1<<TEX_GLOW))
            {
				...
				if(geombatches.length()) {
					renderbatches(cur, RENDERPASS_GLOW);
					glEnableClientState(GL_VERTEX_ARRAY);
				}
                cleanuptexgen(cur);
                glDisableClientState(GL_TEXTURE_COORD_ARRAY);
