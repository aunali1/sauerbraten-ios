...
void generategrass()
{
	...
	if(grassoffsets[0] < 0) {
		loopi(NUMGRASSOFFSETS) grassoffsets[i] = rnd(0x1000000)/float(0x1000000);
	}
    loopi(NUMGRASSWEDGES) {
        grasswedge &w = grasswedges[i];
...
void rendergrass() {
    ...
    loopv(grassgroups) {
        ...
        if(reflecting || refracting) {
			...
			if(isfoggedsphere(g.tri->radius, g.tri->center)) {
				continue;
			}
        }
        if(texid != g.tex) {
        ...
        glDrawArrays(GL_TRIANGLES, g.offset, 6 * g.numquads);
        xtravertsva += 6 * g.numquads;
    }
